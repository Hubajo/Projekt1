...
# pokud uživatel zadá nesprávne přihlašovací udáje vypíše se:    
else:
...
# podmínka výběru textu od 1 do 3, jinak ukonči program
...
for word in words:
    # Odstranění interpunkce a získání jen slovního obsahu
    word = word.strip('.,!?')

    # Pokračovat pouze pro neprázdná slova
    if word:
        # Spočítáme celkový počet slov
        word_count += 1

        # Kontrola, zda slovo začíná velkým písmenem
        if word.istitle():
            title_case_count += 1

        # Kontrola, zda všechna písmena ve slově jsou velká a není zde číslo (pro vynechání 30N apod.)
        if word.isupper() and not word.isdigit() and not any(char.isdigit() for char in word):
            upper_case_count += 1
...
for length in sorted(length_counts):
    ...
    print(f"{counter:3} | {stars:<20} | {length_counts[length]}")
    counter += 1
